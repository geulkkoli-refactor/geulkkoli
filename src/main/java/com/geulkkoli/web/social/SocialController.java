...
public class SocialController {
    ...
    public ModelAndView signUp(@ModelAttribute("signUpDto") SocialSignUpDto signUpDtoUpDto, BindingResult bindingResult, ModelAndView modelAndView) {
        ...
        SocialInfoDto socialInfoDto = SocialInfoDto.of(signUpDto.getAuthorizationServerId(), signUpDto.getClientregistrationName(), user);
    ...
    public ModelAndView disconnect(@RequestParam("type") String socialType, @AuthenticationPrincipal CustomAuthenticationPrinciple authUser) {
        log.info("소셜 연동 해제");
        socialService.disconnect(authUser.getUsername(), socialType);

        ModelAndView modelAndView = new ModelAndView();
        modelAndView.setView(new RedirectView("redirect:/user/edit"));
        ConnectedSocialInfos connectedInfosByEmail = socialInfoFindService.findConnectedInfosByEmail(authUser.getUsername());

        modelAndView.addObject("connectedInfosByEmail", connectedInfosByEmail);
        return modelAndView;
    }
